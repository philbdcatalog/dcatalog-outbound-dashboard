...
export const AE_ROSTER = [
  { name: "Phil Benavides", id: "1937633000476487001", photo: "phil.jpg" },
  { name: "Jay Thankappan", id: "1937633000493432001", photo: "jay.png" },
];

// SDR (call-lane) roster. Identity maps by agent_email on justcall_calls; meeting
// credit is by Zoho owner id via leads.raw->'Meeting_Booked_By'->>'id'. Additive —
// SDRs are a separate lane from the AEs above.
export const SDR_ROSTER = [
  { name: "Noah Drummond", id: "1937633000493438001", email: "noahd@dcatalog.com", photo: "noah.jpg" },
  { name: "Brandon Malave", id: "1937633000493434003", email: "brandonm@dcatalog.com", photo: "brandon.jpg" },
];
export const SDR_ROSTER_NAMES = SDR_ROSTER.map((r) => r.name);

// Zoho owner ids allowed to CREDIT a booked meeting (Meeting_Booked_By) — the
// active SDRs + AEs. Used by the leads sync to ingest SDR-booked leads that fall
// outside the inbound Lead_Source allowlist.
export const MEETING_BOOKED_BY_IDS = [...SDR_ROSTER, ...AE_ROSTER].map((r) => r.id).filter(Boolean);
