...
export function resolvePeriod(param, now = new Date()) {
  ...
  if (param === "last-7-days") {
    // Rolling 7-day window ending now (end exclusive is "now").
    const start = new Date(now.getTime() - 7 * 24 * 60 * 60 * 1000);
    return { value: "last-7-days", start, end: now, label: "last 7 days", isAll: false };
  }
  if (param === "this-year") {
    // end is the NATURAL year end (future) so the delta's prior window lands on
    // last year, not "start − elapsed". No future data exists, so YTD metrics are
    // unaffected.
    return { value: "this-year", start: new Date(Date.UTC(y, 0, 1)), end: new Date(Date.UTC(y + 1, 0, 1)), label: `${y} YTD`, isAll: false };
  }
  if (param === "last-year") {
...
// PACE-ADJUSTED delta window (elapsed-to-elapsed). Compares the SAME elapsed
// slice of the immediately-preceding equivalent period, so a partial current
// period isn't measured against a complete prior one (Q3-to-date vs. the same
// number of weeks into Q2, not all of Q2). One formula covers every option:
//   elapsed    = min(now, periodEnd) - periodStart
//   priorStart = periodStart - fullPeriodLength   (the preceding equivalent period)
//   priorWindow = [priorStart, priorStart + elapsed]
// Returns null when there's no defined prior (all-time) — the indicator hides.
export function priorWindow(period, now = new Date()) {
  ...
  const fullLen = end - start;
  if (!(fullLen > 0)) return null;
  const elapsed = Math.min(now.getTime(), end) - start;
  if (!(elapsed > 0)) return null;
  const priorStart = start - fullLen;
  return { start: new Date(priorStart), end: new Date(priorStart + elapsed) };
}
