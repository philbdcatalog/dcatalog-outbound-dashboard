...
// Delta window mode for the KPI week-over-week chips. "preceding-equal" (default)
// compares the selected period to the immediately preceding window of equal
// length (Last 7 days -> prior 7 days; Current quarter -> last quarter; etc.), so
// the chip is meaningful under every period option. To switch to a fixed calendar
// week (Mon–Sun) vs the prior calendar week regardless of the selector, set this
// to "calendar-week" and compute [mondayOf(now)-7d, mondayOf(now)) vs the week
// before — a one-spot change in priorWindow().
export const DELTA_MODE = "preceding-equal";

// The immediately preceding window of equal length for a resolved period, or null
// when there's no well-defined prior (all-time / open windows) — chips then hide.
export function priorWindow(period) {
