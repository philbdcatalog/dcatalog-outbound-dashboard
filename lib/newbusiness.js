...
export async function getNewBusinessData(window) {
  try {
    ...
    const [dealRes, meetRes, settingsRes, reconRes, roster] = await Promise.all([
      ...
      supabase.from("app_settings").select("nb_meeting_goal, nb_opp_goal, nb_won_goal, stage_probabilities, nb_pipeline_goal_30, nb_pipeline_goal_60, nb_pipeline_goal_90").limit(1).maybeSingle(),
    ...
    // ---- 30/60/90 blocks — anchored to TODAY by fixed day-count windows; these
    // do NOT follow the page period selector (deliberate — forward forecast +
    // pipeline-generation goals). Do not "fix" them to follow the selector.
    const now = new Date();
    const DAY = 86400000;
    const openNow = deals.filter((d) => d.stage === "open"); // same open-deal set the page uses

    // Chart 1 — Expected Revenue: weighted (amount * probability/100) forecast of
    // open deals expected to CLOSE within today+N days (by expected_close_date).
    // Cumulative; OVERDUE open deals (expected_close_date < today) roll into every
    // window (no lower bound). null probability contributes 0 (row not dropped).
    const weightedThrough = (n) => {
      const upper = now.getTime() + n * DAY;
      return openNow
        .filter((d) => d.expected_close_date && new Date(d.expected_close_date).getTime() <= upper)
        .reduce((s, d) => s + amt(d) * ((Number(d.probability) || 0) / 100), 0);
    };
    const expectedRevenue = [30, 60, 90].map((n) => ({ window: n, value: weightedThrough(n) }));
    // Unscheduled open deals (no expected_close_date) can't be placed — surfaced
    // as a footnote (raw unweighted sum) so they don't vanish.
    ...
    // Chart 2 — Pipeline Goals: UNWEIGHTED pipeline GENERATED vs goal. Uses the
    // page's existing pipeline-generated rule (opp_at + full amount, NOT
    // expected_close_date) so it reconciles. Cumulative windows measured from
    // today back over N days — a forward opp_at window would be structurally
    // empty (opp_at is a past milestone), so "pipeline generated in the last N
    // days" is the coherent read of the goal-vs-actual question.
    const generatedInLast = (n) => {
      const lo = now.getTime() - n * DAY;
      return deals
        .filter((d) => d.opp_at && (() => { const t = new Date(d.opp_at).getTime(); return t >= lo && t <= now.getTime(); })())
        .reduce((s, d) => s + amt(d), 0);
    };
    const pipelineGoals = [30, 60, 90].map((n) => {
      const goal = Number(settings[`nb_pipeline_goal_${n}`]) || 0;
      const actual = generatedInLast(n);
      return { window: n, goal, actual, pct: goal > 0 ? Math.round((actual / goal) * 100) : null };
    });
    const plan306090 = { expectedRevenue, unscheduled, pipelineGoals };
