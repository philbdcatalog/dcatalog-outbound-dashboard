...
export async function getNewBusinessData(window) {
  try {
    ...
    const quarterStartISO = currentQuarter(new Date()).startISO;
    ...
    const cycleDeals = wonAll.filter((d) => d.won_at && d.created_at);
    const avgCycleDays = cycleDeals.length
      ...
      : null;
    // Cohort = deals that entered pipeline (opp_at) in the period; reached >= stage.
    const stageTable = NB_STAGE_ORDER.map((stage, i) => ({
      stage,
      prob: typeof probs[stage] === "number" ? probs[stage] : null,
      reached: wOpp.filter((d) => stageIdx(d.stage_detail) >= i).length,
    }));
    const sales = { avgDealSize, avgCycleDays, stageTable };
