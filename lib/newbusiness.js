...
// Avg-deal-size floor: exclude auto-subscribe / monthly deals ($29–$600) that
// have no distinguishing Zoho flag. One-line change if the band shifts.
export const AVG_DEAL_MIN = 1000;
...
export async function getNewBusinessData(window) {
  try {
    ...
    const [dealRes, meetRes, settingsRes, reconRes, roster] = await Promise.all([
      fetchAll(() => supabase.from("deals").select("account_id, company_name, stage, stage_detail, amount, source, source_channel, expected_close_date, probability, tool, created_at, meeting_at, opp_at, won_at, closed_at, raw, accounts(domain, company_name)")),
      fetchAll(() => supabase.from("meetings").select("account_id, domain, booked_at, held, meeting_status, source, source_channel, tool, channel, owner_name:raw->>owner_name, accounts(domain, company_name)")),
      supabase.from("app_settings").select("nb_meeting_goal, nb_opp_goal, nb_won_goal, stage_probabilities, nb_pipeline_goal_m1, nb_pipeline_goal_m2, nb_pipeline_goal_m3").limit(1).maybeSingle(),
      supabase.from("zoho_recon_queue").select("id", { count: "exact", head: true }).eq("status", "pending"),
      loadNewBusinessOwners(supabase).catch(() => ({ ids: new Set(), nameById: new Map() })),
    ]);
    ...
    // New Business is roster-owned: a meeting counts only when it's credited to a
    // new_business_owner_ids rep (by meeting owner_name, else the account's deal
    // owner) OR it's an outbound/SDR-booked meeting (source='outbound' — credits
    // Noah/Brandon, who aren't in the AE roster). Non-roster inbound (e.g. Matt
    // Moberly) is excluded here. (The Inbound tab still shows all inbound.)
    const rosterNames = new Set([...(roster?.nameById?.values?.() || [])].filter(Boolean));
    ...
    const meetingCounts = (m) => {
      if (m.source === "outbound") return true;
      const owner = m.owner_name || ownerByAccount.get(m.account_id);
      return !!(owner && rosterNames.has(owner));
    };

    // Period-scoped views (ever-reached).
    const wMeetings = meetings.filter((m) => inWin(m.booked_at) && meetingCounts(m));
    ...
    // Avg deal size / cycle use ALL won deals (all-time). Won deals are keyed by
    // closed status (stage='won'), NOT by create date — create date is only
    // needed for cycle time (and is missing on some imports). Auto-subscribe /
    // monthly deals ($29–$600, no Zoho flag) are excluded via AVG_DEAL_MIN so
    // they don't drag the average down; real deals start ~$2,500.
    const wonAll = deals.filter((d) => d.stage === "won");
    const sizeDeals = wonAll.filter((d) => amt(d) >= AVG_DEAL_MIN);
    const avgDealSize = sizeDeals.length ? sizeDeals.reduce((s, d) => s + amt(d), 0) / sizeDeals.length : 0;
    ...
    const openDealsList = openDeals
      .map((d) => {
        ...
        return {
          company: nameOf(d), rep: ownerOf(d) || "—", stage: d.stage_detail || "—", amount: amt(d), rank: idx < 0 ? 99 : idx,
          expectedClose: d.expected_close_date ?? null,
          closePct: d.probability ?? null,
          expectedAmount: amt(d) * ((Number(d.probability) || 0) / 100),
    ...
    const openExpectedTotal = openDealsList.reduce((s, d) => s + d.expectedAmount, 0);

    const sales = { avgDealSize, avgCycleDays, stageTable, openDeals: openDealsList, openTotal, openExpectedTotal };
    ...
    // ---- Month-of-quarter blocks — the 3 calendar months of the CURRENT quarter
    // (from currentQuarter(), auto-rolling Q3 Jul/Aug/Sep -> Q4 Oct/Nov/Dec).
    // Anchored to the current quarter — these do NOT follow the page period
    // selector (deliberate). Do not "fix" them to follow the selector.
    const cq = currentQuarter(new Date());
    const qStartMonth = cq.start.getUTCMonth();
    const qYear = cq.start.getUTCFullYear();
    const qStartMs = cq.start.getTime();
    const qEndMs = Date.UTC(qYear, qStartMonth + 3, 1);
    const quarterMonths = [0, 1, 2].map((i) => {
      const startMs = Date.UTC(qYear, qStartMonth + i, 1);
      const endMs = Date.UTC(qYear, qStartMonth + i + 1, 1);
      return { key: `m${i + 1}`, label: new Date(startMs).toLocaleString("en-US", { month: "short", timeZone: "UTC" }), startMs, endMs };
    });
    const openNow = deals.filter((d) => d.stage === "open"); // same open-deal set the page uses

    // Chart 1 — Expected Revenue by month: weighted (amount * probability/100) of
    // open deals whose expected_close_date falls in that calendar month. Null
    // probability contributes 0 (row not dropped).
    const expectedRevenue = quarterMonths.map((mo) => ({
      label: mo.label,
      value: openNow
        .filter((d) => { if (!d.expected_close_date) return false; const t = new Date(d.expected_close_date).getTime(); return t >= mo.startMs && t < mo.endMs; })
        .reduce((s, d) => s + amt(d) * ((Number(d.probability) || 0) / 100), 0),
    }));
    // Footnotes: open deals that can't be placed in a quarter-month (raw unweighted).
    const unscheduledDeals = openNow.filter((d) => !d.expected_close_date);
    const unscheduled = { count: unscheduledDeals.length, amount: unscheduledDeals.reduce((s, d) => s + amt(d), 0) };
    const beyondDeals = openNow.filter((d) => { if (!d.expected_close_date) return false; const t = new Date(d.expected_close_date).getTime(); return t < qStartMs || t >= qEndMs; });
    const beyondQuarter = { count: beyondDeals.length, amount: beyondDeals.reduce((s, d) => s + amt(d), 0) };

    // Chart 2 — Pipeline Goals by month: UNWEIGHTED full amount of deals whose
    // pipeline-add date (opp_at, else created_at) falls in that calendar month —
    // the page's pipeline-generated rule, so it reconciles. Future months read $0
    // and the current month is partial by design (no special-casing).
    const addDateMs = (d) => { const s = d.opp_at || d.created_at; if (!s) return NaN; const t = new Date(s).getTime(); return isNaN(t) ? NaN : t; };
    const pipelineGoals = quarterMonths.map((mo, i) => {
      const goal = Number(settings[`nb_pipeline_goal_m${i + 1}`]) || 0;
      const actual = deals
        .filter((d) => { const t = addDateMs(d); return t >= mo.startMs && t < mo.endMs; })
        .reduce((s, d) => s + amt(d), 0);
      return { label: mo.label, goal, actual, pct: goal > 0 ? Math.round((actual / goal) * 100) : null };
    });
    const plan306090 = { expectedRevenue, unscheduled, beyondQuarter, pipelineGoals };
