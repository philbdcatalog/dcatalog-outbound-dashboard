...
export async function getDashboardData(window) {
  ...
  // Current calendar quarter (UTC), from the shared helper — used ONLY to scope
  // the recon-queue pending badge (which resets each quarter, independent of the
  // dashboard period selector). The metric windows come from `window` below.
  const now = new Date();
  const cq = currentQuarter(now);
  const quarterStartISO = cq.startISO;
  ...
  const [touchRes, acctRes, meetRes, dealRes, campRes, reconRes, settingsRes] = await Promise.all([
    ...
    // Pending count scoped to this quarter (matches the recon-queue view).
