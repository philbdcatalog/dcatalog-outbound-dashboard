...
export async function getDashboardData() {
  ...
  // Current calendar quarter (UTC) — the board resets each quarter. Used to
  // scope the "Output This Quarter" gauges, the recent feeds, and the recon-
  // queue pending badge to this quarter's activity.
  const now = new Date();
  ...
  const [touchRes, acctRes, meetRes, dealRes, campRes, reconRes, settingsRes] = await Promise.all([
    ...
    fetchAllRows(() => supabase.from("deals").select("account_id, company_name, stage, amount, is_outbound, closed_at, tool, raw, accounts(last_channel)")),
