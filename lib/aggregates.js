...
export async function getDashboardData() {
  ...
  // Current calendar quarter (UTC), from the shared helper — the board resets
  // each quarter. Used to scope the "Output This Quarter" gauges, the account-
  // based funnel, the recent feeds, and the recon-queue pending badge.
  const now = new Date();
  const cq = currentQuarter(now);
  const quarterStart = cq.start;
  ...
  // ACCOUNT-BASED FUNNEL — quarter-scoped (resets each quarter like the gauges),
  // each stage on its OWN natural date. The all-time sets above stay unscoped for
  // the by-tool / by-rep / by-campaign / trend sections below. Dedupe-by-account
  // is preserved; we just add the current-quarter date filter on top.
  const inQ = (s) => { if (!s) return false; const d = new Date(s); return !isNaN(d.getTime()) && d >= quarterStart; };
  const funnel = {
    contacted: new Set(touches.filter((t) => inQ(t.occurred_at)).map((t) => t.account_id)).size,
    replied: new Set(
      touches.filter((t) => (t.kind === "reply" || t.kind === "interested") && inQ(t.occurred_at)).map((t) => t.account_id)
    ).size,
    meetings: new Set(meetings.filter((m) => inQ(m.booked_at)).map((m) => m.account_id)).size,
    opps: new Set(
      dealsJoined.filter((d) => d.stage === "open" && d.is_outbound && inQ(d.created_at)).map((d) => d.account_id)
    ).size,
    won: new Set(
      dealsJoined.filter((d) => d.stage === "won" && d.is_outbound && inQ(d.closed_at)).map((d) => d.account_id)
    ).size,
  };
  ...
  // Outbound closed revenue THIS QUARTER: stage=won AND is_outbound AND closed
  // in the current quarter. (quarterStart is the UTC calendar-quarter start
  // computed above.) A null/invalid closed_at falls outside the quarter and is
  // excluded. This is the value behind the "Outbound Won" gauge.
  const inQuarter = (s) => {
    if (!s) return false;
    const dt = new Date(s);
    return !isNaN(dt.getTime()) && dt >= quarterStart;
  };
  const outboundWon = dealsJoined
    .filter((d) => d.stage === "won" && d.is_outbound && d.amount && inQuarter(d.closed_at))
    .reduce((s, d) => s + Number(d.amount), 0);

  // Pipeline Generated: outbound opportunities GENERATED this quarter — sum of
  // amount for OPEN, is_outbound deals whose Zoho Created_Time (read from the
  // stored raw payload) falls in the current quarter. Open only: won has its own
  // gauge and lost counts toward neither. This is the value behind the 4th gauge.
  const pipelineGenerated = dealsJoined
    .filter((d) => d.is_outbound && d.stage === "open" && d.amount && inQuarter(d.raw?.Created_Time))
    .reduce((s, d) => s + Number(d.amount), 0);

  // Quarter-scoped counts for the "Output This Quarter" gauges (the board resets
  // each quarter). Meetings -> booked THIS quarter; Opps -> outbound OPEN deals
  // created THIS quarter (by Zoho Created_Time, not close date). Distinct
  // accounts, mirroring the funnel's company-based counting.
  const meetingsThisQuarter = new Set(
    meetings.filter((m) => inQuarter(m.booked_at)).map((m) => m.account_id)
  ).size;
  const oppsThisQuarter = new Set(
    dealsJoined
      .filter((d) => d.is_outbound && d.stage === "open" && inQuarter(d.raw?.Created_Time))
      .map((d) => d.account_id)
  ).size;
  ...
  const recent = {
    // Scoped to this quarter (the board resets each quarter): meetings booked
    // this quarter; open opps generated (Created_Time) this quarter.
    meetings: [...meetings]
      .filter((m) => inQuarter(m.booked_at))
      .map((m) => ({ domain: domainOf(m.account_id), date: m.booked_at, channel: channelTag(m.account_id, m.channel) }))
      .sort(dateDesc)
      .slice(0, 8),
    opps: outboundDeals
      .filter((d) => d.stage === "open" && inQuarter(d.raw?.Created_Time))
      ...
      .sort(dateDesc)
      .slice(0, 8),
    // Wins feed: EVERY outbound won deal closed this quarter (not a recent-N
    // slice), each carrying its amount for currency display.
    won: outboundDeals
      .filter((d) => d.stage === "won" && inQuarter(d.closed_at))
      .map((d) => ({
        domain: wonLabel(d),
        date: d.closed_at,
  ...
  return {
    ok: true,
    funnel,
    // All-time distinct totals — the By Channel table's Total row stays all-time
    // (its per-tool rows are all-time), independent of the quarter-scoped funnel.
    funnelAllTime: {
