...
export async function getDashboardData(window) {
  ...
  const [touchRes, acctRes, meetRes, dealRes, campRes, reconRes, settingsRes] = await Promise.all([
    ...
    fetchAllRows(() => supabase.from("deals").select("account_id, company_name, stage, amount, is_outbound, closed_at, created_at, tool, raw, accounts(last_channel)")),
  ...
  // Core sets, ALL scoped to the selected period: touches/meetings are already
  // period-filtered; the deal sets carry their own per-stage date filter (opps by
  // created_at, won by closed_at). These feed the funnel AND By Tool / By Rep /
  // By Channel / By Campaign, so all of those are period-scoped too.
  ...
  const wonSet = new Set(
    dealsJoined.filter((d) => d.stage === "won" && d.is_outbound && inWindow(d.closed_at)).map((d) => d.account_id)
  );
  const oppsSet = new Set(
    dealsJoined.filter((d) => d.stage === "open" && d.is_outbound && inWindow(d.created_at)).map((d) => d.account_id)
  );
  ...
  // Gauge metrics — scoped to the SELECTED PERIOD (the window from the selector).
  // Outbound Won = sum(amount) won+is_outbound closed in window; Pipeline
  // Generated = sum(amount) open+is_outbound created in window (deals.created_at);
  // Meetings / Opps counts mirror the funnel (distinct accounts, same window).
  const outboundWon = dealsJoined
    .filter((d) => d.stage === "won" && d.is_outbound && d.amount && inWindow(d.closed_at))
    .reduce((s, d) => s + Number(d.amount), 0);
  const pipelineGenerated = dealsJoined
    .filter((d) => d.is_outbound && d.stage === "open" && d.amount && inWindow(d.created_at))
    .reduce((s, d) => s + Number(d.amount), 0);
