...
export async function getDashboardData(window) {
  ...
  // Known roster — always shown, even at zero. Then any rep present in the data
  // (touch rep_names OR deal owners) so we never hide a real rep.
  const ROSTER = ["Traci Vrana", "Phil Benavides", "Jonathan Marin"];
  const reps = [
    ...new Set([
      ...ROSTER,
      ...touches.map((t) => t.rep_name).filter(Boolean),
