...
import { AE_ROSTER_NAMES } from "./roster";
...
function getServiceClient() {
  ...
  return createClient(url, serviceKey, {
    ...
    // Force every Supabase read to bypass Next's fetch data cache. The page
    // segment is already force-dynamic / fetchCache:"force-no-store", but pinning
    // cache:"no-store" on the client's own fetch guarantees freshly-synced rows
    // (e.g. a new won deal) are never served from a cached fetch — defense in
    // depth against the documented stale-dashboard-read issue.
    global: {
...
const QUARTER_GOALS = { meetings: 75, opps: 30, pipeline: 350000, won: 150000 };
...
export async function getDashboardData(window) {
  ...
  const [touchRes, acctRes, meetRes, dealRes, campRes, reconRes, settingsRes] = await Promise.all([
    ...
    fetchAllRows(() => supabase.from("accounts").select("id, domain, company_name, last_channel, first_touch_at")),
    fetchAllRows(() => supabase.from("meetings").select("account_id, channel, tool, booked_at, source")),
    fetchAllRows(() => supabase.from("deals").select("account_id, company_name, stage, amount, is_outbound, closed_at, created_at, meeting_at, opp_at, won_at, tool, raw, accounts(last_channel)")),
    fetchAllRows(() => supabase.from("campaigns").select("id, name, channel")),
    // Pending count = the whole backlog (matches the recon-queue view, which is
    // NOT period-scoped — pending items persist until worked, across quarters).
    supabase.from("zoho_recon_queue").select("id", { count: "exact", head: true }).eq("status", "pending"),
    supabase.from("app_settings").select("meeting_goal, opps_goal, pipeline_goal, won_goal, cost_email, cost_linkedin, cost_phone, cost_multichannel").limit(1).maybeSingle(),
  ]);
  ...
  const goals = {
    ...
    won: Number(settings.won_goal) || QUARTER_GOALS.won,
  };

  const allTouches = touchRes.data || [];
  const accounts = acctRes.data || [];
  // OUTBOUND dashboard: exclude meetings explicitly tagged inbound/other-sourced
  // (they belong to the Inbound page). NULL/legacy-untagged and 'outbound' stay,
  // preserving prior behavior for historical meetings. Applies to both the
  // period-scoped counts and the full-history trend chart below.
  const allMeetings = (meetRes.data || []).filter((m) => m.source !== "inbound" && m.source !== "other");
  const dealsJoined = dealRes.data || [];
  const campaigns = campRes.data || [];

  // Period window from the selector ({ start, end } | null; null start = all time).
  const win = window || {};
  const inWindow = (s) => {
  ...
  // Period-scoped views (touches by occurred_at, meetings by booked_at) that feed
  // the funnel / By Tool / By Rep / By Channel / feeds / gauges. The trend charts
  // deliberately use the FULL arrays (allTouches / allMeetings) further down.
  const touches = win.start ? allTouches.filter((t) => inWindow(t.occurred_at)) : allTouches;
  const meetings = win.start ? allMeetings.filter((m) => inWindow(m.booked_at)) : allMeetings;

  const acctById = new Map(accounts.map((a) => [a.id, a]));

  // Core sets, ALL scoped to the selected period. EVER-REACHED counting: a deal
  // counts in EVERY milestone it passed through in the window, via its milestone
  // timestamps (meeting_at / opp_at / won_at), independent of its CURRENT stage —
  // so a created+won-same-day deal still shows up under Meetings and Opps.
  // Touches feed contacted/replied; the meetings table adds meetings that never
  // became deals. These sets feed the funnel AND By Tool / By Rep / By Channel /
  // By Campaign, so all of those are period-scoped + ever-reached too.
  const outDeals = dealsJoined.filter((d) => d.is_outbound);
  ...
  const meetingSet = new Set([
    ...meetings.map((m) => m.account_id), // meetings table (booked_at already in window)
    ...outDeals.filter((d) => inWindow(d.meeting_at)).map((d) => d.account_id),
  ]);
  const oppsSet = new Set(outDeals.filter((d) => inWindow(d.opp_at)).map((d) => d.account_id));
  const wonSet = new Set(outDeals.filter((d) => inWindow(d.won_at)).map((d) => d.account_id));

  // ACCOUNT-BASED FUNNEL — the selected period, each stage on its own natural
  // date. Dedupe-by-account preserved.
  const funnel = {
  ...
  // (now / quarterStart are computed once at the top of getDashboardData.)
  const COST_MAP = [
  ...
  for (const mm of meetings) {
    if (!mm.booked_at) continue; // `meetings` is already scoped to the period
  ...
  // By Rep: per-rep performance. Meetings/Opps/Wins credit the OUTREACH rep
  // (rep_name of the account's last meaningful touch) when one exists, else the
  // Zoho DEAL OWNER — so inbound meetings (no outreach touch) still credit the AE
  // who ran them instead of no one (repForRecord, shared with the AE dashboard).
  // Accounts/Replies still count the rep's own touch activity directly.
  const lmtRepOf = (id) => lmt.get(id)?.rep;
  // account_id -> Zoho deal owner_name (first non-null), for the fallback credit.
  ...
  const repOf = (id) => repForRecord(lmtRepOf(id), ownerByAccount.get(id));
  // Display the ACTIVE roster only — do NOT union in data owners, or departed
  // reps (who still own historical deals) would reappear as rows. Attribution
  // (repOf) is unchanged; this is purely which reps show.
  const reps = [...AE_ROSTER_NAMES];
  const byRep = reps
    .map((rep) => {
      ...
      const meetings = [...meetingSet].filter((id) => repOf(id) === rep).length;
      const opps = [...oppsSet].filter((id) => repOf(id) === rep).length;
      const wins = [...wonSet].filter((id) => repOf(id) === rep).length;
  ...
  // Gauge metrics — SELECTED PERIOD + EVER-REACHED (milestone timestamps). A deal
  // contributes to Pipeline when it reached opp (opp_at in window) and to Won when
  // it reached won (won_at in window), regardless of its current stage — so a
  // won deal still counts its amount under Pipeline Generated.
  const outboundWon = outDeals
    .filter((d) => d.amount && inWindow(d.won_at))
    .reduce((s, d) => s + Number(d.amount), 0);
  const pipelineGenerated = outDeals
    .filter((d) => d.amount && inWindow(d.opp_at))
    .reduce((s, d) => s + Number(d.amount), 0);
  const meetingsThisQuarter = meetingSet.size;
  const oppsThisQuarter = oppsSet.size;

  // Week-over-week deltas vs the immediately-preceding equal-length window. Reuses
  // the already-fetched data (no extra queries); null when there's no prior
  // (all-time) so the chips hide.
  ...
  const priorMeetings = new Set([
    ...allMeetings.filter((m) => inPrior(m.booked_at)).map((m) => m.account_id),
    ...outDeals.filter((d) => inPrior(d.meeting_at)).map((d) => d.account_id),
  ]).size;
  const priorOpps = new Set(outDeals.filter((d) => inPrior(d.opp_at)).map((d) => d.account_id)).size;
  const priorPipeline = outDeals.filter((d) => d.amount && inPrior(d.opp_at)).reduce((s, d) => s + Number(d.amount), 0);
  const priorWon = outDeals.filter((d) => d.amount && inPrior(d.won_at)).reduce((s, d) => s + Number(d.amount), 0);
  const hasPrior = !!pw;
  const deltas = {
    meetings: makeDelta(meetingsThisQuarter, priorMeetings, hasPrior),
    opps: makeDelta(oppsThisQuarter, priorOpps, hasPrior),
    pipeline: makeDelta(pipelineGenerated, priorPipeline, hasPrior),
    won: makeDelta(outboundWon, priorWon, hasPrior),
  };
  ...
  // Display name for a won deal: a win must never render blank. Prefer the
  // account's company_name, then the deal's own company_name (set by the Zoho
  // sync), then the account domain, and only "—" if truly nothing is known.
  const wonLabel = (d) =>
    acctById.get(d.account_id)?.company_name || d.company_name || acctById.get(d.account_id)?.domain || "—";
  ...
  const recent = {
    // Scoped to the selected period: meetings booked, open opps created, wins
    // closed — all within the window. (`meetings` is already period-filtered.)
    meetings: [...meetings]
      .map((m) => ({ domain: domainOf(m.account_id), date: m.booked_at, channel: channelTag(m.account_id, m.channel) }))
      .sort(dateDesc)
      .slice(0, 8),
    opps: outboundDeals
      .filter((d) => d.stage === "open" && inWindow(d.created_at))
      // Open opps have no close date; show when the opp was generated instead.
      .map((d) => ({ domain: domainOf(d.account_id), date: d.closed_at || d.raw?.Created_Time, channel: channelTag(d.account_id, d.accounts?.last_channel) }))
      .sort(dateDesc)
      .slice(0, 8),
    // Wins feed: every outbound won deal closed within the selected period,
    // each carrying its amount for currency display.
    won: outboundDeals
      .filter((d) => d.stage === "won" && inWindow(d.closed_at))
      .map((d) => ({
        domain: wonLabel(d),
        date: d.closed_at,
        amount: d.amount != null ? Number(d.amount) : null,
        channel: channelTag(d.account_id, d.accounts?.last_channel),
      }))
      .sort(dateDesc),
  };

  // ---- 2) Meetings & Opps over time (monthly, last 6) ---------------------
  // TREND CHARTS use FULL history and IGNORE the period selector (they exist to
  // show history across periods). Bar = meetings booked that bucket (allMeetings);
  // darker = those whose account is an outbound opp/won deal ever (allOppAccounts).
  const allOppAccounts = new Set(
    dealsJoined.filter((d) => (d.stage === "open" || d.stage === "won") && d.is_outbound).map((d) => d.account_id)
  );
  const meetingsBucket = (keyOf) => (bucket) => {
    const inBucket = allMeetings.filter((mm) => keyOf(mm.booked_at) === bucket.key);
    return {
      label: bucket.label,
      meetings: inBucket.length,
      opps: inBucket.filter((mm) => allOppAccounts.has(mm.account_id)).length,
    };
  };
  ...
  const accountsBucket = (keyOf) => (bucket) => {
    // Trend chart -> full history (allTouches), independent of the period selector.
    const total = new Set(
      allTouches.filter((t) => keyOf(t.occurred_at) === bucket.key).map((t) => t.account_id)
    ).size;
  ...
  return {
    ...
    totals: { accounts: accounts.length, touches: allTouches.length },
    recent,
