...
export function mapHeyReachEvent(eventType) {
  ...
  const norm = eventType.trim().toUpperCase();
  switch (norm) {
    ...
    case "LEAD_AUTO_TAGGED_INTERESTED":
      return { kind: "interested", meaningful: true };
    // Negative / neutral auto-tags: no positive kind in our enum — skip them
    // explicitly so the "INTERESTED" substring fallback below never mislabels
    // a NOT_INTERESTED lead as interested.
    case "LEAD_AUTO_TAGGED_NOT_INTERESTED":
    case "LEAD_AUTO_TAGGED_GENERIC":
      return null;
    default:
      break; // fall through to the normalized fallback below
  }

  // Normalized fallback for HeyReach naming variants the explicit cases miss —
  // e.g. the "Every Message/InMail Reply Received" webhook sends an event type
  // (EVERY_MESSAGE_REPLY_RECEIVED / EVERY_INMAIL_REPLY_RECEIVED /
  // FIRST_MESSAGE_REPLY_RECEIVED, etc.) that wasn't matched above, which was
  // silently skipping reply rows. Order matters: check REPLY before SENT so an
  // event that contains both words resolves to a reply.
  if (norm.includes("REPLY")) return { kind: "reply", meaningful: true };
  if (norm.includes("ACCEPTED")) return { kind: "connected", meaningful: false };
  // Guard against negated variants ("NOT INTERESTED") matching as interested.
  if (norm.includes("INTERESTED") && !norm.includes("NOT")) return { kind: "interested", meaningful: true };
  if (norm.includes("SENT")) return { kind: "sent", meaningful: false };
  return null; // genuinely unknown event: skip in v1
}
...
export function mapJustCallEvent(eventType, callInfo) {
  ...
  //
  // Observed real call_info.type values are exactly "Connected" and
  // "Not Connected" (plus the doc sample "Outgoing human answered"). We must
  // handle the NEGATED form: "Not Connected" contains the substring "connected",
  // so a naive /connected/ test wrongly marks a missed dial as connected. So:
  // anything negated (contains "not connected" or starts with "not") is an
  // attempt -> "sent"; otherwise a real "answer"/"connected" -> "connected".
  const outcome = (
    ...
    .trim()
    .toLowerCase();
  const negated = outcome.includes("not connected") || outcome.startsWith("not");
  const connected = isAnswered || (!negated && /(answer|connected)/.test(outcome));
