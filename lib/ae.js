  ...
  try {
    ...
    const openDeals = deals
      ...
      .map((d) => ({
        ...
        expectedAmount: num(d.amount) * ((Number(d.probability) || 0) / 100),
    ...
    // Never let the catch itself throw: a non-Error rejection (err = null/string)
    // would make err.message throw here, rejecting getAEData's promise and
    // propagating out of the page's await as an uncaught RSC exception (the raw
    // Next digest page). Coerce defensively so the page always gets {ok:false}.
