...
export async function getAEData(window, repName) {
  try {
    ...
    const [dealRes, meetRes, touchRes, leadRes, goalRes] = await Promise.all([
      ...
      fetchAll(() => supabase
        .from("meetings")
        .select("account_id, source, source_channel, channel, tool, booked_at, domain, accounts(domain, company_name)")),
      fetchAll(() => supabase
        .from("touch_events")
    ...
    const perRep = AE_ROSTER.map(({ name, photo }) => {
      const repMeetings = meetings.filter((m) => inWin(m.booked_at) && repOfAccount(m.account_id) === name);
    ...
    const openDeals = deals
      ...
      .map((d) => ({ company: nameOfDeal(d), rep: repOfAccount(d.account_id), stage: d.stage_detail || d.raw?.Stage || "Open", amount: num(d.amount) }))
