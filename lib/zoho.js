...
// List ALL records from a module via the standard records GET endpoint
// (GET /crm/v8/{module}), covered by ZohoCRM.modules.READ — unlike /coql (needs
// coql.READ, which our token lacks) and unlike /search where `not_equal` on a
// picklist fails closed. It CANNOT filter by arbitrary criteria, so the caller
// filters client-side after fetch. `fields` is required (comma-separated API
// names). READ-ONLY.
//
// Pagination: simple page/per_page offset paging is capped at 2,000 records
// total ("DISCRETE_PAGINATION_LIMIT_EXCEEDED"). Our deal set is ~2,554, so we
// MUST use CURSOR pagination: the first response's info.next_page_token is
// passed back as `page_token` on the next request, and so on. Once a
// next_page_token exists we page by TOKEN ONLY (never send page + page_token
// together). Stop when more_records is false / no token.
export async function zohoListAll({ accessToken, module, fields, perPage = 200, maxPages = 30 }) {
  const out = [];
  let pageToken = null;
  ...
  for (let i = 0; i < maxPages; i++) {
    ...
    if (fields) params.set("fields", fields);
    params.set("per_page", String(perPage));
    // First page: no token (page defaults to 1). Subsequent pages: token only.
    if (pageToken) params.set("page_token", pageToken);

    const url = `${ZOHO_API_BASE}/crm/v8/${module}?${params.toString()}`;
    const res = await fetchWithTimeout(url, {
      headers: { Authorization: `Zoho-oauthtoken ${accessToken}` },
      cache: "no-store",
    });
    ...
    if (!res.ok) {
      const text = await res.text().catch(() => "");
      throw new Error(`Zoho ${module} list failed (${res.status}): ${text}`);
    }
    ...
    const info = json.info || {};
    if (!info.more_records || !info.next_page_token || batch.length === 0) break; // last page
...
// Run a COQL query, paging through ALL matching records. Returns an array.
// COQL (POST /crm/v8/coql) is more capable than the /search criteria API: it
// supports `IN (...)` lists and quoted string literals containing slashes
// (e.g. 'Proposal/Negotiation'), so we can filter Owner + Stage server-side in
// one query — unlike the /search criteria, where `(Stage:not_equal:Closed Won)`
// fails closed and a slashed stage value is ignored. `baseQuery` MUST NOT carry
// its own LIMIT/OFFSET — we append `limit/offset` for paging. READ-ONLY.
export async function zohoCoqlAll({ accessToken, baseQuery, perPage = 200, maxPages = 50 }) {
  const out = [];
  for (let page = 0; page < maxPages; page++) {
    const select_query = `${baseQuery} limit ${perPage} offset ${page * perPage}`;
    const res = await fetchWithTimeout(`${ZOHO_API_BASE}/crm/v8/coql`, {
      method: "POST",
