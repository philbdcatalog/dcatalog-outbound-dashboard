...
// (GET /crm/v8/{module}), paging by page/per_page + info.more_records. This
// endpoint is covered by ZohoCRM.modules.READ — unlike /coql (needs coql.READ,
// which our token lacks) and unlike /search where `not_equal` on a picklist
// fails closed. It CANNOT filter by arbitrary criteria, so the caller filters
// client-side after fetch. `fields` is required (comma-separated API names).
// READ-ONLY.
export async function zohoListAll({ accessToken, module, fields, perPage = 200, maxPages = 100 }) {
  const out = [];
  for (let page = 1; page <= maxPages; page++) {
    const params = new URLSearchParams();
    if (fields) params.set("fields", fields);
    ...
    const info = json.info || {};
    if (!info.more_records || batch.length === 0) break; // last page
  }
