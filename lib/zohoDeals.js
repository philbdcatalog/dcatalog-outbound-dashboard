...
// Empty/missing -> null; a present numeric string -> Number (0–100), else null.
...
export async function buildDealWritePatch(supabase, existingRow, fields, milestone = {}) {
  ...
  // Forecast fields from Zoho raw — written on EVERY deal write (NOT in the
  // is_outbound-preserving exclusion). Only touch a column when the Zoho field is
  // PRESENT in raw, so a webhook payload lacking Probability never nulls an
  // existing value; a present-but-empty value maps to NULL.
  const rawObj = patch.raw && typeof patch.raw === "object" ? patch.raw : null;
  if (rawObj) {
    if (patch.expected_close_date === undefined && has(rawObj, "Closing_Date")) {
      patch.expected_close_date = emptyToNull(rawObj.Closing_Date);
    }
    if (patch.probability === undefined && has(rawObj, "Probability")) {
      patch.probability = numOrNull(rawObj.Probability);
    }
  }
