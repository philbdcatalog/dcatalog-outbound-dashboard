...
export const LEADS_FIELDS =
  "id,Company,Full_Name,Email,Website,Lead_Source,Lead_Status,Lifecycle_Stage,Meeting_Status,Meeting_Booked_By,New_Date,Working_Date,MQL_Date,SQL_Date,Disqualified_Date,Owner,Created_Time,Modified_Time";
...
export function classifyInboundLead(lead, floorMs) {
  ...
  if (!bucket && !bookedByRoster) return { keep: false };
...
// Materialize a `meetings` row for an SDR-booked COLD-CALL lead, so it counts on
// the Outbound dashboard (gauge/funnel/by-channel). Only for the JustCall SDR
// lane (source_channel='other' + Meeting_Booked_By set) — inbound leads with a
// booker are left to the inbound/deal path. Idempotent on external_id
// 'lead-meeting:<zoho_lead_id>'; adopts the manually-materialized row rather than
...
export async function ensureMeetingForBookedLead(supabase, row) {
  try {
    const raw = row.raw || {};
    const mbb = raw.Meeting_Booked_By;
    if (!mbb || mbb.id == null) return { created: false, reason: "no meeting_booked_by" };
    if (row.source_channel !== "other") return { created: false, reason: "not sdr call lane" };
    const domain = row.domain;
    if (!domain) return { created: false, reason: "no domain" };
    const bookedRaw = row.created_at || raw.Modified_Time || null;
