...
export async function ensureMeetingForBookedLead(supabase, row, seen) {
  try {
    const raw = row.raw || {};
    ...
    // In-memory batch guard: at most one meeting per domain+quarter per sync pass
    // (the DB check below races when two leads in the same pass share a
    // domain+quarter, since neither is committed when the other checks).
    const qKey = `${domain}|${when.getUTCFullYear()}-Q${Math.floor(when.getUTCMonth() / 3) + 1}`;
    if (seen && seen.has(qKey)) return { created: false, reason: "batch dup (domain+quarter)" };
    ...
    if (existing && existing.length) {
      if (seen) seen.add(qKey);
      return { created: false, reason: "domain+quarter already has a meeting" };
    }
