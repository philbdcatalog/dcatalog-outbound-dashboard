...
export function mapCallRow(call) {
  ...
  return {
    ...
    product: "phone", // phone-number product (matches the column default)
...
// ---- Sales Dialer (predictive campaigns) — SEPARATE product/API -------------
// This is where the SDRs' actual outbound dialing lives (the /calls endpoint is
// the phone-number product, mostly inbound strays). Shaped differently: no nested
// call_duration — call_info.duration IS the conversation time. History limit:
// the Sales Dialer API only serves the LAST 3 MONTHS, so a deeper backfill than
// ~3 months back returns nothing (our July-1 floor is well within range).
export function mapSalesDialerRow(call) {
  const id = pick(call, ["call_id", "id", "call_sid"]);
  if (id == null) return null;

  const email = pick(call, ["agent_email", "agent.email"]);
  // SD call_date/call_time are documented as already-UTC — combine, don't shift.
  let occurredISO = null;
  if (call.call_date && call.call_time) {
    const d = new Date(`${call.call_date}T${call.call_time}Z`);
    if (!isNaN(d.getTime())) occurredISO = d.toISOString();
  ...
  return {
    // Prefixed — SD and phone call-id namespaces are separate and could collide.
    justcall_call_id: `sd:${String(id)}`,
    product: "sales_dialer",
    agent_id: call.agent_id == null ? null : String(call.agent_id),
    agent_name: call.agent_name ?? null,
    agent_email: email ? String(email).toLowerCase() : null,
    direction: pick(call, ["call_info.direction", "direction"]),
    disposition: pick(call, ["call_info.disposition", "disposition"]), // raw; blank -> null
    duration_sec: numOrNull(pick(call, ["call_info.duration", "duration"])),
    conversation_sec: numOrNull(pick(call, ["call_info.duration", "duration"])), // SD: duration IS talk time
    acw_sec: null, // not provided by this endpoint
    vm_dropped: false, // not provided; do NOT infer from Machine Pickup
    occurred_at: occurredISO,
    raw: call,
  };
}

// Fetch the Sales Dialer call log for [sinceISO, now], paged ascending. Uses
// last_call_id_fetched to guarantee no duplicates across pages (the API supports
// it for exactly this). Stops when a page underfills.
export async function fetchSalesDialerCalls({ sinceISO, perPage = 100, maxPages = 60 }) {
  const out = [];
  const headers = { Authorization: authHeader(), Accept: "application/json" };
  ...
  for (let page = 0; page < maxPages; page++) {
    ...
    if (sinceISO) params.set("from_datetime", sinceISO.slice(0, 19).replace("T", " "));
    if (lastId != null) params.set("last_call_id_fetched", String(lastId));

    const url = `${JUSTCALL_API_BASE}/sales_dialer/calls?${params.toString()}`;
    const res = await fetchWithTimeout(url, { headers, cache: "no-store" });
    if (res.status === 429) break;
    if (!res.ok) {
      const text = await res.text().catch(() => "");
      throw new Error(`JustCall sales_dialer fetch failed (${res.status}): ${text.slice(0, 300)}`);
    }
    const json = await res.json().catch(() => ({}));
    const batch = Array.isArray(json.data) ? json.data : Array.isArray(json.calls) ? json.calls : Array.isArray(json) ? json : [];
    out.push(...batch);
    ...
    lastId = pick(last, ["call_id", "id", "call_sid"]);
