...
export async function getInboundData(window) {
  try {
    ...
    const [dealRes, meetRes, leadRes, reconRes, settingsRes] = await Promise.all([
      ...
      // respect the 1000-row select cap. meeting_status is pulled from raw
      // (Zoho Meeting_Status; no dedicated column) for the held / show-rate read.
      fetchAll(() => supabase
        .from("leads")
        .select("created_at, lead_status, source_channel, new_at, mql_at, sql_at, meeting_status:raw->>Meeting_Status")),
    ...
    // Inbound tab reads ONLY true inbound channels — the `leads` table now also
    // holds SDR-booked non-inbound rows (source_channel 'other'), which must NOT
    // inflate inbound counts. Exclude 'other'/'unknown'.
    const INBOUND_CHANNELS = new Set(["google_ads", "facebook_ads", "website", "linkedin", "trade_show"]);
    const leads = (leadRes && !leadRes.error ? leadRes.data || [] : []).filter((l) => INBOUND_CHANNELS.has(l.source_channel));
    ...
    // Meeting held tracking from Zoho Meeting_Status (rep-filled). Booked signal
    // stays Lead_Status='Meeting Booked'; Performed=held, Need to Reschedule=no-show.
    const booked = leadsInWin.filter((l) => l.lead_status === "Meeting Booked").length;
    const held = leadsInWin.filter((l) => l.meeting_status === "Performed").length;
    const noShow = leadsInWin.filter((l) => l.meeting_status === "Need to Reschedule").length;
    const leadsBlock = {
