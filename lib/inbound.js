...
export async function getInboundData(window) {
  try {
    ...
    // Funnel — period-scoped, ever-reached. Leads = distinct inbound accounts
    // created / met in the window. MQL/SQL pending the Zoho lifecycle field.
    const inboundAccounts = new Set(
      [...wDealsCreated.map((d) => d.account_id), ...wMeetings.map((m) => m.account_id), ...dealMeeting.map((d) => d.account_id)].filter(Boolean)
    );
    const funnel = {
    ...
    const recent = {
      ...
      opps: dealOpp.map((d) => ({ name: nameOfDeal(d), date: d.opp_at || d.closed_at || d.raw?.Created_Time, channel: d.source_channel || "unknown" })).sort(dateDesc).slice(0, 8),
