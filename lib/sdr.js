...
// NOTE: no ACW helper — the Sales Dialer API returns no after-call-work field
// (acw_sec is null on every row), so any ACW card would be a permanent false 0m.
...
export async function getSDRData(window, sdrName) {
  try {
    ...
    const perSdr = SDR_ROSTER.map(({ name, id, email, photo }) => {
      ...
      // Total connected-call time = sum of duration over ALL dials (the SD API
      // returns one flat duration; conversation_sec duplicates it, so there is no
      // separate talk-time). Shown as "Total Call Time", not talk time.
      const talkSec = myCalls.reduce((s, c) => s + durationOf(c), 0);
      // Avg call duration is only meaningful over REAL conversations — averaging
      // over all dials drags in ~70% no-answers/voicemails (a misleading ~0:19).
      // Reuse the Brief 7 isConnect predicate so this stays in sync with connects.
      const convCalls = myCalls.filter(isConnect);
      const convDurationSec = convCalls.reduce((s, c) => s + durationOf(c), 0);
      ...
      return {
        ...
        quota,
        talkSec,
        connectsForAvg: convCalls.length,
        convDurationSec,
        avgConvSec: convCalls.length ? convDurationSec / convCalls.length : 0,
        vmDropped,
    ...
    const team = {
      ...
      // Weighted avg conversation duration = total conversation time / total real
      // conversations across scope (a single figure, not an average-of-averages).
      avgConvSec: sumN("connectsForAvg") ? sumN("convDurationSec") / sumN("connectsForAvg") : 0,
