...
// Industry values that mean "we don't actually know" — these rows are noise and
// are dropped on client import.
const BAD_INDUSTRY = new Set(["needs review", "not found"]);

export default function ClientUpload({ C }) {
  ...
  // Parse CSV -> normalized, domain-deduped client rows + skipped count.
  // Hygiene (locked): skip + count rows with (a) no usable domain, (b) a domain
  // string sitting in the industry column (a comma-shift artifact), or (c) an
  // industry of "needs review" / "not found".
      ...
      Papa.parse(f, {
        ...
        complete: (res) => {
          ...
          for (const row of res.data) {
            ...
            if (!domain) { skipped++; continue; } // (a) no usable domain

            const industryRaw = clean(row["industry"]);
            // (b) comma-shift artifact: a domain ended up in the industry column.
            if (industryRaw && normalizeDomain(industryRaw)) { skipped++; continue; }
            // (c) junk industry labels.
            if (industryRaw && BAD_INDUSTRY.has(industryRaw.toLowerCase())) { skipped++; continue; }
            ...
            byDomain.set(domain, {
              ...
              vertical: clean(row["vertical"] ?? row["Vertical"]),
  ...
  async function onUpload() {
    ...
    try {
      ...
      setResult(json);
      setBusy(false);
      router.refresh(); // re-load the footprint metrics
    } catch (err) {
  ...
  const radio = (val, label, hint) => (
    <label style={{ display: "flex", gap: 8, alignItems: "flex-start", cursor: "pointer", fontSize: 13 }}>
      <input type="radio" name="clientmode" checked={mode === val} onChange={() => setMode(val)} disabled={busy} />
      <span>
  ...
  return (
    <div style={{ background: C.panel, borderRadius: 14, border: `1px solid ${C.line}`, padding: 20, boxShadow: SHADOW }}>
      <div style={{ fontSize: 13, fontWeight: 700, color: C.navy, marginBottom: 12 }}>Upload Clients CSV</div>

      <div style={{ display: "flex", flexDirection: "column", gap: 8, marginBottom: 14 }}>
        {radio("add", "Add to existing clients", "upsert on domain — update matches, add new, keep the rest")}
        {radio("replace", "Replace all clients", "wipe all client rows, then import this file")}
      </div>
      ...
      >
        ...
        <div style={{ fontSize: 13.5, color: file ? C.ink : C.inkSoft, fontWeight: file ? 600 : 400 }}>
          {file ? file.name : "Drag your clients CSV here, or click to browse"}
        </div>
        {file && (
          <div style={{ fontSize: 12, color: C.muted, marginTop: 3 }}>
            Ready to {mode === "replace" ? "replace all clients" : "add to clients"}
          </div>
        )}
      </div>
      ...
      {result && (
        <div style={{ color: C.ink, fontSize: 13, marginTop: 12 }}>
          Imported {(result.inserted + result.updated).toLocaleString()} clients
          {" "}({result.inserted.toLocaleString()} new, {result.updated.toLocaleString()} updated,
          {" "}{result.skipped.toLocaleString()} skipped — no domain / bad industry).
        </div>
      )}
    </div>
  );
